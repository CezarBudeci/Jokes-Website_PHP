...
if ($sort == 'oldest') {
    $sql = "SELECT * FROM posts WHERE topic $category ORDER BY createdon asc LIMIT $offset, $no_of_records_per_page";
//Gets the count of likes on a single post
} else if ($sort == 'highest') {
    $sql = "SELECT posts.*, COUNT(ratinginfo.postid) as likes
            FROM posts LEFT JOIN ratinginfo ON posts.postid = ratinginfo.postid WHERE topic $category GROUP BY postid ORDER BY likes DESC
            LIMIT $offset, $no_of_records_per_page";
//Otherwise result to newest posts
} else {

    $sql = "SELECT * FROM posts WHERE topic $category ORDER BY createdon desc LIMIT $offset, $no_of_records_per_page";
}
//END
...
<!--                    Comment part -->
                        ...
                        <?php
                            if ($_SESSION['role'] == 'admin') {?>
                                <button class="delPostbtn12" data-id="<?php echo $post['postid'] ?>" data-bs-toggle="modal" data-bs-target="#staticBackdrop1">Delete post</button>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                          ...
                                          <div class="modal-footer">
                                            ...
                                            <button type="button" class="delPostbtn2" data-bs-dismiss="modal">Delete Post</button>
                                          </div>
                                        </div>
                                    </div>
                                </div>
